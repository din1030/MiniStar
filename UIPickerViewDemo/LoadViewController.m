...
-(void) getData:(NSData *) data{
    ...
    _starComment = [_json objectForKey:@"starComment"]; // 剝第一層（把每個comment 存進 array）
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section
{
    ...
    return _starComment.count;
}

- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    static NSString *CellIdentifier = @"cells";
    LoadViewCell *cell = [tableView
    ...
    NSDictionary* temp = _starComment[row];
    
    
    
    cell.name.text = [temp objectForKey:@"name"];
    cell.name.font = [UIFont systemFontOfSize:14.0];
    cell.name.frame =  CGRectMake(80, 30, 100, 20);
    ...
    cell.zodiac.text = zodiac_Str;
    cell.zodiac.font = [UIFont systemFontOfSize:15.0];
    cell.zodiac.frame =  CGRectMake(200, 30, 100, 20);
    cell.zodiac.hidden = YES;
    
    cell.comment.text = [temp objectForKey:@"content"];
    cell.comment.font = [UIFont systemFontOfSize:14.0];
    cell.comment.frame =  CGRectMake(80, 40, 190, 50);
    ...
    if ([sex_Str isEqualToString:@"g"]) {
        if ([zodiac_Str isEqualToString:@"1"]) {
            [cell.star_icon.subviews makeObjectsPerformSelector:@selector(removeFromSuperview)];
    ...
    [cell.star_icon addSubview:subview];
